    ...
    if users:
        for i, user in enumerate(users):
            ...
            accept = InlineKeyboardButton(
                text="Принять", callback_data=f"accept_{user.user_id}"
            )
            reject = InlineKeyboardButton(
                text="Отклонить", callback_data=f"reject_{user.user_id}"
            )

            keyboard = InlineKeyboardMarkup(inline_keyboard=[[accept, reject]])
...
async def all_users_handler(message: types.Message):
    ...
    if users:
        ...
        if len(user_list) > max_message_length:
            chunks = [
                user_list[i : i + max_message_length]
                for i in range(0, len(user_list), max_message_length)
            ]
            for chunk in chunks:
...
async def reload_catalog(message: types.Message):
    try:
        ...
        await message.answer(
            f"Возникла ошибка❌\nСкорее всего файл не найден\nНажмите кнопку '➕Изменить каталог' и добавьте новый каталог или поместите файл в bot/utils/data/catalog/data.xlsx"
        )
