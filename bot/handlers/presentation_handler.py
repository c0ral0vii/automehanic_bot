...
async def send_presentations(message: types.Message):
    try:
        ...
        buttons = [[InlineKeyboardButton(text=pdf, callback_data=pdf.replace(" ", "_"))] for pdf in pdf_files]
        keyboard = InlineKeyboardMarkup(inline_keyboard=buttons)

        await message.answer("Выберите презентацию:", reply_markup=keyboard)
    except Exception as e:
        await message.answer(f'Не удалось отправить презентации, повторите попытку, отправьте это разработчику {e}')
