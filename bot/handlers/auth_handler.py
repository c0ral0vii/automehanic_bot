    ...
    if current_state == AuthForm.name:
        if isinstance(callback_query, types.CallbackQuery):
            await callback_query.message.answer(
                'Предыдущего шага нет, или введите имя или нажмите "Отмена":',
...
async def process_name(message: types.Message, state: FSMContext):
    ...
    if not name:
        await message.answer(
            "Имя не может быть пустым. Пожалуйста, введите корректное имя:"
        )
        return
...
async def process_surname(message: types.Message, state: FSMContext):
    ...
    if not surname:
        await message.answer(
            "Фамилия не может быть пустой. Пожалуйста, введите корректную фамилию:"
        )
        return
...
async def process_organization_name(message: types.Message, state: FSMContext):
    ...
    if not organization_name:
        await message.answer(
            "Название организации не может быть пустым. Пожалуйста, введите корректное название:"
        )
        return
    ...
    keyboard = create_inline_navigation_keyboard()
    await message.answer(
        "Введите ваш номер телефона (например, +79991234567):", reply_markup=keyboard
    )
...
async def process_phone_number(message: types.Message, state: FSMContext):
    ...
    if not validate_phone_number(phone_number):
        await message.answer(
            "Некорректный номер телефона. Пожалуйста, введите номер в формате +79991234567:"
        )
        return
    ...
    if user_added:
        await message.answer(
            "Спасибо за запрос на авторизацию! Ваше сообщение отправлено, администратор скоро его обработает. Ожидайте подтверждения.",
            reply_markup=keyboard,
        )
    else:
        await message.answer(
            "Ваша заявка уже находиться на рассмотрении у модераторов",
            reply_markup=keyboard,
        )
