...
from database.db_config import (
    get_all_users,
    get_user,
    get_users_with_role_cancelled,
    get_users_with_role_user,
    get_users_with_role_undefined,
    update_user_price_level,
    update_user_role,
)
...
async def change_one_user_handler(message: types.Message, state: FSMContext):
    ...
    await message.answer(
        "Выберите пользователя для изменения уровня цен:", reply_markup=keyboard
    )
    await state.set_state(AdminStates.waiting_for_one_user_id)
...
async def change_group_handler(message: types.Message, state: FSMContext):
    await message.answer(
        "Введите список ID пользователей (каждый с новой строки) для изменения уровня."
    )
...
async def change_category_handler(message: types.Message, state: FSMContext):
    await message.answer(
        "Введите номер категории пользователей, которой нужно изменить уровень. (1. Авторизованный, 2. В процессе обработки модераторами, 3. Не прошедшие модерацию)"
    )
...
async def change_all_users_handler(message: types.Message, state: FSMContext):
    await message.answer(
        "Вы уверены, что хотите поменять уровень для всех пользователей? Отправьте 'Да' для подтверждения."
    )
...
@admin_router.callback_query(
    ...
    StateFilter(AdminStates.waiting_for_one_user_id),
)
async def process_select_user(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    await callback_query.message.edit_text(
        user_info, reply_markup=create_simple_inline_navigation()
    )
    await state.set_state(AdminStates.waiting_for_price_selection)
...
async def process_group_user_ids(message: types.Message, state: FSMContext):
    ...
    await message.answer(
        f"Получены ID пользователей: {', '.join(user_ids)}. Теперь введите новый уровень цен.",
...
async def process_category_selection(message: types.Message, state: FSMContext):
    ...
    await message.answer(
        f"Вы выбрали категорию: {category}. Теперь введите новый уровень цен.",
        reply_markup=create_simple_inline_navigation(),
    )
    await state.set_state(AdminStates.waiting_for_price_selection)
...
async def process_all_users_selection(message: types.Message, state: FSMContext):
    ...
    if confirmation == "Да":
        ...
        await message.answer(
            "Введите новый уровень цен для всех пользователей",
            reply_markup=create_simple_inline_navigation(),
        )
        await state.set_state(AdminStates.waiting_for_price_selection)
    else:
        await message.answer(
            "Вы вернулись в админ-панель", reply_markup=create_admin_navigation()
        )
        await state.clear()
...
async def process_price_selection(message: types.Message, state: FSMContext):
    ...
    if "user_id" in data:
        ...
        if result:
            await message.answer(
                f"Уровень цен для пользователя с ID {user_id} успешно изменён на уровень {price_level}."
            )
        else:
        ...
        for user_id in user_ids:
            ...
            if result:
                ...
                await message.answer(
                    f"Не удалось изменить уровень цен для пользователя с ID {user_id}."
                )
        ...
        for user in users:
            result = await update_user_price_level(
                int(user.user_id), selected_price_level
            )
            if result:
                await message.answer(
                    f"Уровень цен для пользователя {user.name} {user.surname} успешно изменён на уровень {price_level}."
                )
            else:
                await message.answer(
                    f"Не удалось изменить уровень цен для пользователя {user.name} {user.surname}."
                )
    ...
    await state.clear()
    await message.answer(
        "Изменение уровня цен завершено.", reply_markup=create_admin_navigation()
    )
