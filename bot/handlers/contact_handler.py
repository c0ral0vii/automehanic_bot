...
from keyboards.inline.contact_keyboard import (
    create_contact_keyboard,
    create_inline_navigation_keyboard,
)
...
async def handle_single_article_request(
    ...
    await callback_query.message.answer(
        "Введите ваш вопрос:", reply_markup=create_inline_navigation_keyboard()
    )
...
async def handle_question(message: types.Message, state: FSMContext):
    ...
    await message.answer(
        "Спасибо за ваш вопрос! Мы получили ваше сообщение и постараемся ответить на него в ближайшее время.",
        reply=keyboard,
    )
    await state.clear()
