...
async def add_products_from_excel():
    ...
    df = df[['Исходный номер', 'Наименование', 'Наличие, шт.', 'Розничная цена, руб.', 'Уровень цен 1, руб.',
             ...
             'Тип запчасти', 'Ссылка на фото 1', 'Ссылка на фото 2', 'Ссылка на фото 3', 'Ссылка на фото 4 (чертеж)', 'Кросс-номера',
             'Техника', 'Вес, кг.', 'Длина, м.', 'Внутренний диаметр, мм.',
             'Внешний диаметр, мм.', 'Диаметр резьбы', 'Ширина, м.', 'Высота, м.']]
    ...
    for index, row in df.iterrows():
        ...
        default_price = str(row['Розничная цена, руб.']) if not pd.isnull(row['Розничная цена, руб.']) else 'Напишите напишите нам, чтобы узнать цену'
        first_lvl_price = str(row['Уровень цен 1, руб.']) if not pd.isnull(row['Уровень цен 1, руб.']) else 'Напишите напишите нам, чтобы узнать цену'
        second_lvl_price = str(row['Уровень цен 2, руб.']) if not pd.isnull(row['Уровень цен 2, руб.']) else 'Напишите напишите нам, чтобы узнать цену'
        third_lvl_price = str(row['Уровень цен 3, руб.']) if not pd.isnull(row['Уровень цен 3, руб.']) else 'Напишите напишите нам, чтобы узнать цену'
        fourth_lvl_price = str(row['Уровень цен 4, руб.']) if not pd.isnull(row['Уровень цен 4, руб.']) else 'Напишите напишите нам, чтобы узнать цену'
        ...
        photo_url_4 = str(row['Ссылка на фото 4 (чертеж)']) if not pd.isnull(row['Ссылка на фото 4 (чертеж)']) else None
